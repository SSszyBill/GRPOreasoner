...
Thin CLI that delegates all real work to the `evaluation` package.

Usage (no-GPU smoke test):
    python -m evaluation \\
        --config experiments/4090_grpo_multisource_v1/configs/eval_frozen.yaml \\
        --model  Qwen/Qwen2.5-1.5B \\
        --working-dir /root/autodl-tmp/rl_playground \\
        --skip-generation

Usage (GPU full run, writes results to disk):
    python -m evaluation \\
        --config experiments/4090_grpo_multisource_v1/configs/eval_frozen.yaml \\
        --model  /root/autodl-tmp/.../snapshots/<hash> \\
        --working-dir /root/autodl-tmp/rl_playground \\
        --out experiments/4090_grpo_multisource_v1/results/baseline \\
        --tag qwen25_1_5b_base
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", required=True,
                        help="Path to eval_frozen.yaml")
    parser.add_argument("--model", required=True,
                        help="Model path (local snapshot dir) or HF repo id")
    parser.add_argument("--working-dir", default=".",
                        help="Base dir for relative parquet paths in yaml")
    parser.add_argument("--out", required=False,
                        help="Output dir for baseline / post-training results "
                             "(creates <out>/<timestamp>_<tag>/)")
    parser.add_argument("--tag", required=False, default="untagged",
                        help="Run tag, used in output subfolder name")
    parser.add_argument("--skip-generation", action="store_true",
                        help="Only run config + data loader; skip vLLM (no-GPU smoke test)")
...
def main() -> None:
    ...
    for eval_set_name, samples in eval_data_by_set.items():
        ...
        # Section 5: aggregate to 5 metrics.
        eval_set_spec = next(
    ...
    # Section 5: write outputs if --out was given.
    if args.out:
