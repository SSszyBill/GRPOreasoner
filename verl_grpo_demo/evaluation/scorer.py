"""Route each SampleGeneration through utils.reward_multi and decompose the
...
from evaluation.generator import SampleGeneration
...
class ScoredCompletion:
    ...
    correctness: float   # 1.0 if answer correct (per reward_multi), else 0.0
    format_ok: float     # 1.0 if output format matches (#### / \boxed{}), else 0.0
    total_reward: float  # correctness + 0.1 * format_ok, matches training reward
...
class ScoredGeneration:
    ...
    - greedy_score  : single score for pass@1 (from greedy completion)
    - sampled_scores: k scores for maj@8 / format_rate (from k sampled completions)
    """
    index: int
    data_source: str
    ground_truth: str
...
def score_completion(
    ...
    Two calls to reward_multi.compute_score with different weights extract each
    component in isolation. See module docstring for rationale.
    """
...
def score_generations(
    ...
    extra_info_by_index: optional {sample.index: extra_info} for data sources
    that need it (e.g., countdown needs numbers / target). GSM8K and MATH500
    do not need this; leave as None for the standard eval.
    """
